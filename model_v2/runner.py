...
def run(
    ...
    for fold, (train_ids, val_ids) in enumerate(splits.split(all_x, all_y_binary)):
        ...
        MIL_FLAG = model_name == 'DSepMIL'

        if DSEPEMB_FLAG:
            CustomLoss = TripletLoss(device)
        if MIL_FLAG:
            MILLoss = ComboFocalF1Loss(alpha=-1.0, gamma=2.0, weight_f1=0.5)
        ...
        for epoch in pbar:
            ...
            for i, data in enumerate(train_loader):
                ...
                if DSEPEMB_FLAG:
                    ...
                    apneic_loss = MILLoss(apneic_stage, apneic_groundtruth)
                else:
            ...
            for i, val_data in enumerate(val_loader):
                ...
                if DSEPEMB_FLAG:
                    ...
                    v_apneic_loss = MILLoss(est_val_apneic, val_apneic)
                else:
...
def forward(
    ...
    if model_name == "DSepEmbedder":
        ...
        test_pred = []
        # 顺带逐 batch 收集 MIL 注意力权重（仅当 model 暴露 last_attention 时）
        batch_attn_list = []
        ...
        with torch.no_grad():
            for i, test_batch in enumerate(test_loader):
                ...
                # MIL 模型在 forward 内部会刷新 last_attention，DataParallel 包装时位于 model.module
                inner = model.module if hasattr(model, "module") else model
                attn = getattr(inner, "last_attention", None)
                if attn is not None:
                    batch_attn_list.append(attn.detach().cpu().numpy())
    ...
    # 拼接所有 batch 的 attention 矩阵；非 MIL 模型则返回 None
    test_attention = (
        np.concatenate(batch_attn_list, axis=0) if batch_attn_list else None
    )

    return test_pred.cpu().numpy(), inference_time, test_pred_prob, test_attention
